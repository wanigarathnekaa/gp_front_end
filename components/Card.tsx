...
const Card = ({title, description,icon :Icon, wide, onclick }:CardProps) => {
  return (
    <div className={`p-6 mb-8 mt-15 ml-3 rounded-2xl border bg-white shadow-md flex items-center  group cursor-pointer transform transition-transform duration-300 hover:scale-105 hover:text-blue-500 h-13 ${wide ? 'w-full' : 'w-6/12'}`}
      onClick={onclick}
    >
